...
function run_tests {
  ...
  OS_TEST_PATH=`echo $testrargs|grep -o 'apic_ml2\.tests[^[:space:]:]*\+'|tr . /`
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='apic_ml2/*' --omit='apic_ml2/openstack/common/*' -d covhtml -i
  fi
